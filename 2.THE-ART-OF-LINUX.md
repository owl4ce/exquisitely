...
> Ensure the internet connection is available. Authenticate to wireless network with iwctl if you are using wireless
> connection, or skip this if the ethernet cable is plugged properly.
> 
> ```sh
> iwctl
> ```
...
Remove unnecessary OpenRC services.

```sh
doas rc-update del agetty.tty2 default
doas rc-update del agetty.tty3 default
doas rc-update del agetty.tty4 default
doas rc-update del agetty.tty5 default
doas rc-update del agetty.tty6 default
doas rc-update del net.lo default
doas rc-update del net.lo boot
doas rc-update del netmount default
doas rc-update del local default
...
<img align="center" width="100%" src="./src-images/Screenshot_20251013_234805.png"/>

> ```sh
> doas pacman -S pipewire pipewire-audio pipewire-pulse
> ```
> ```sh
> doas pacman -S plasma kde-applications iio-sensor-proxy bluez-openrc
> ```
> ```sh
> doas rc-update add bluetoothd default
> ```
> ```sh
> doas chmod -c u+s /usr/lib/iio-sensor-proxy
> ```
> ```sh
...
> [ -f ~/.bashrc ] && . ~/.bashrc
> 
> if [ -z "$WAYLAND_DISPLAY" -a "${XDG_VTNR:-0}" -eq 1 ]; then
>     pipewire >/dev/null 2>&1 &
>     pipewire-pulse >/dev/null 2>&1 &
>     wireplumber >/dev/null 2>&1 &
>     /usr/lib/iio-sensor-proxy >/dev/null 2>&1 &
